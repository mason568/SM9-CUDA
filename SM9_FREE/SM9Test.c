...
//===========签名用到的参数==========
    ...
    unsigned char id[5] = {"\x41\x6C\x69\x63\x65"};//Alice
...
// ========================================================
    ...
    /* begin 打印签名主公钥msk 和mspk*/
    printf("签名主密钥msk,i.e.,ks：\n");
    print_hex(msk.msk,msk.keylen);
    printf("主签名公钥mspk,也叫签名主公钥,i.e.,P_[pub-s]：\n");
    print_hex(mspk.x1,32);
    print_hex(mspk.x2,32);
    print_hex(mspk.y1,32);
    print_hex(mspk.y2,32);
    /* end   打印签名主公钥msk 和mspk*/
    ...
    printf("用于生成签名私钥ds_[A]的用户id=Alice,i.e.,ID_[A]:\n");
    print_hex(pk.pk,pk.keylen);
    SM9_GenSignSecKey(&sk, &pk, &msk); // 由公钥（id）生成签名私钥
 
    printf("生成签名私钥,i.e.,ds_[A]:\n");
    print_hex(sk.x,32);
    print_hex(sk.y,32);
...
//===============signature test===============
    ...
    printf("待签名消息Chinese IBS standard,i.e.,M:\n");
